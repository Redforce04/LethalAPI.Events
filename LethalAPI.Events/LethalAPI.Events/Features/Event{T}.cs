// -----------------------------------------------------------------------
// <copyright file="Event{T}.cs" company="Redforce04">
...
public class Event<T> : ILethalApiEvent
{
    ...
    {
        Log.Debug($"Event {typeof(T).Name} Invoked", Plugin.Instance.Config.LogEventPatching, "LethalAPI-Events");
        ...
        foreach (CustomEventHandler<T> handler in this.InnerEvent.GetInvocationList().Cast<CustomEventHandler<T>>())
        {
            try
            {
                handler(arg);
